...
# 
# usage:
# ./multi_batch_render.sh MXS_DIR OUTPUT_DIR NUM_NODES SL_START SL_STEP SL_STOP [PROJECT_NAME]
# 
...
# 
# Require: multi_render_thread.sh
...
InitMXSFileList () {
  MXS_FILE_LIST_PATH=$(mktemp ./logs/${PROJNAME}_tmp_batch_render.XXXXXXXXXX) || { echo "Failed to create temp file"; exit 1; }
...
StartRenderJob () {
  ...
  qsub -N "${PROJNAME}_thread_${THREAD_NUM}"  -l walltime=48:00:00 \
    ...
    -v MXS_FILE_LIST_PATH=${MXS_FILE_LIST_PATH},OUTPUT_DIR=${OUTPUT_DIR},SL_START=${SL_START},SL_STEP=${SL_STEP},SL_STOP=${SL_STOP},MXS_DIR=${MXS_DIR},RES=${RES} \
